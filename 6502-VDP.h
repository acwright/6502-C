/* =============================================================================
 *   6502-VDP.h — AC6502 system header for C programs on the ACE with a
 *                6502-PICOVDP
 * =============================================================================
 *
 *   CPU:       WDC 65C02S
 *   Video:     6502-PICOVDP (SPEC draft 0.5)
 *   BIOS:      v2.0
 *   Compiler:  cc65
 *
 *   The C counterpart of 6502-VDP.inc.  Include it to reach the Kernal
 *   routines, the IO hardware registers, the system variables and the
 *   constants that go with them.  The names, the addresses and the section
 *   order all match 6502-VDP.inc, so the two can be read side by side.
 *
 *   An ACE with a TMS9918A on BIOS 1.x uses 6502.h and 6502.lib instead.  A
 *   program built with this header links 6502-VDP.lib, and only runs on an ACE
 *   with a 6502-PICOVDP on BIOS 2.x.  To build one source both ways, select
 *   the header with the VDP macro (make VDP=1 passes -D VDP):
 *
 *       #ifdef VDP
 *       #include "6502-VDP.h"
 *       #else
 ...
 *       #endif
 *
 *   Names follow 6502-VDP.inc's rule: a value the PICOVDP SPEC defines is
 *   VC_*, and a name the BIOS defines (a Kernal entry, a RAM variable) is
 *   BIOS.inc's own.
 *
 ...
 *
 *   Everything outside that is the BIOS's, and 6502-VDP.inc documents it in full.
 *
 *   CALLING CONVENTION
 *   ------------------
...
#ifndef _AC6502_VDP_H
#define _AC6502_VDP_H

#ifdef _AC6502_H
#error "6502-VDP.h and 6502.h describe different machines: include only one"
#endif
...
#define BRK_PTR             (*(void (**)(void))0x0302)    /* BRK handler vector (default: the BRK report, then BASIC) */
...
#define HW_VID              0x80          /* Bit 7: Video card    (IO 8, a 6502-PICOVDP with the built-in font) */

/* IO_MODE values: */
#define IO_MODE_VIDEO       0x00          /* Console output goes to the 6502-PICOVDP */
#define IO_MODE_SERIAL      0x01          /* Console output goes to the R65C51 */

/* BRK register save area.  The BRK handler fills it, prints a report and
 * warm-starts BASIC.  BASIC's SYS also leaves the called routine's A, X, Y and P
 * in BRK_A, BRK_X, BRK_Y and BRK_P. */
...
#define VDP_FW              (*(volatile unsigned char *)0x0394)  /* STAT5 (firmware version, BCD) at boot; 0 if no PICOVDP */
#define VDP_CAPS            (*(volatile unsigned char *)0x0395)  /* STAT6 (VC_CAP_* bits) at boot; 0 if no PICOVDP */
#define VDP_L0CTRL_SHADOW   (*(volatile unsigned char *)0x0396)  /* What was last written to L0CTRL through the Kernal */
#define VDP_L1CTRL_SHADOW   (*(volatile unsigned char *)0x0397)  /* What was last written to L1CTRL through the Kernal */
#define VDP_P0              (*(volatile unsigned char *)0x0398)  /* Parameter block for VdpSprite / VdpSetScroll (the wrappers set it) */
 ...
 * =============================================================================
 * One wrapper per entry point, named exactly as 6502-VDP.inc names it.  Each is
 * a __fastcall__ function in 6502-VDP.lib that shuffles arguments into the
 * registers the routine wants and JSRs the jump table.
 *
 ...
 * XFER_PTR / CF_BUF_PTR / CF_LBA macros above, or from assembly via 6502-VDP.inc.
 */
...
/* --- Video console (6502-PICOVDP) --- */
/* KernalInit leaves the card in its legacy submode; the first Chrout,
 * VideoClear, VideoSetCursor, VideoGetCursor or VideoSetColor brings the 40x24
 * Text console up (InitVideo, then a clear). */

void InitVideo(void);
/* Put the card in the Text console: writes the Text layout with the display
 * off, has the card reload its built-in font into $0800 and waits for it
 * (returns after the next vertical blank), restores palette row 0, sets the
 * border from the pen and turns the display on.  Does not clear the screen.
 * Writes IRQEN = 0, so re-enable a VDP interrupt afterwards.  Restores the
 * console in full after a program has changed modes, tables or glyphs. */

void VideoClear(void);
/* Fill the screen with spaces in the pen, put the cursor back at (0,0) and
 * the scroll origin back to row 0. */

void __fastcall__ VideoPutChar(char c);
/* Write a character, coloured with the pen, at the cursor without advancing
 * the cursor. */
...
/* Scroll up one line in hardware (VID_TOP, L0SCRY) and clear the bottom row
 * in the pen. */

void __fastcall__ VideoSetColor(unsigned char color);
/* Set the pen, (foreground << 4) | background in TMS_* palette row 0 colours,
 * for text printed from now on, and the border, which follows the background.
 * Text already on screen keeps its colours. */
 ...
 * and a checksum per slot (layout in 6502-VDP.inc and the BIOS README).  All
 * six check for the RTC themselves. */
...
/* --- PICOVDP (v2.0) --- */
/* Port A only, leaving VBANK = 0 and VINC = +1.  Each one does nothing when
 * there is no PICOVDP console (HW_VID clear) or an argument is out of range,
 * and says so in its return value.  Only these entries keep VID_MODE and the
 * LxCTRL shadows; a program writing VC.reg directly keeps its own records. */

unsigned int VdpInfo(void);
/* What the boot probe found, packed as (VDP_CAPS << 8) | VDP_FW — see
 * VDP_INFO_FW() and VDP_INFO_CAPS() — or 0 if there is no PICOVDP console. */

#define VDP_INFO_FW(v)      ((unsigned char)((v) & 0xFF))
#define VDP_INFO_CAPS(v)    ((unsigned char)((v) >> 8))

unsigned char __fastcall__ VdpWriteReg(unsigned char reg, unsigned char value);
/* Write register reg (0-127, VC_REG_*).  A VMODE write updates VID_MODE, an
 * L0CTRL/L1CTRL write its shadow.  Returns 1 if there is no card or reg > 127. */

unsigned char __fastcall__ VdpSetMode(unsigned char mode);
/* Write VMODE, VC_VMODE_TEXT to VC_VMODE_FULL (1-4), and VID_MODE with it.
 * The register only: tables, layers and sprites are the caller's to lay out,
 * and the Text console is InitVideo().  Returns 1 if no card or out of range. */

unsigned char __fastcall__ VdpPoke(unsigned int addr, unsigned char value);
/* Write one byte anywhere in the 64 KB of VRAM.  Returns 1 if no card. */

unsigned char __fastcall__ VdpPeek(unsigned int addr);
/* Read one byte anywhere in VRAM.  Returns 0 if no card. */

unsigned char __fastcall__ VdpSetPalette(unsigned char entry, unsigned int rgb);
/* Set palette entry 0-255 to rgb = 0x0RGB (4 bits each), at VC_PALETTE + 2 *
 * entry, where InitVideo puts PALBASE.  Returns 1 if no card. */

unsigned char WaitVBlank(void);
/* Return at the start of the next vertical blank, by polling STAT3: STAT0's
 * flags and the STAT1 latches are left alone.  With no card it waits 2 cs
 * and returns 1. */

unsigned char __fastcall__ VdpLoadFile(const char *name, unsigned int vaddr);
/* Load a named file from the current disk into VRAM at vaddr, exactly its
 * FS_FILE_SIZE bytes.  Returns 1 if no card, not found, or a read error. */

unsigned char __fastcall__ VdpLoadFont(unsigned char font);
/* Load a built-in font into layer 0's pattern table (L0PAT x $800) and wait
 * for it to land (the next vertical blank).  VC_FONT_CP437 (0) is the only
 * font.  Returns 1 if no card or another font. */

typedef struct {
    unsigned char y;                      /* VDP_P0: Y */
    unsigned char x;                      /* VDP_P1: X bits 7:0 */
    unsigned char pattern;                /* VDP_P2: pattern */
    unsigned char attr;                   /* VDP_P3: VC_ATTR_* (b7 = X bit 8) */
} VdpSpriteAttr;

unsigned char __fastcall__ VdpSprite(unsigned char sprite, const VdpSpriteAttr *attr);
/* Write sprite 0-63's four attribute bytes, into the sprite attribute table
 * at VRAM $2000 (SPRATTR = $40, where BASIC's SCREEN 1-3 put it); after moving
 * SPRATTR, use VdpPoke.  Marks VID_MODE disturbed.  Returns 1 if no card or
 * sprite > 63. */

unsigned char __fastcall__ VdpSetScroll(unsigned char layer, unsigned int x,
                                        unsigned char y);
/* Scroll layer 0-1 to x (0-511) and y: writes LxSCRX, LxSCRY and LxCTRL's
 * VC_LCTRL_SCRX8, keeping LxCTRL's other bits.  Marks VID_MODE disturbed.
 * Returns 1 if no card or layer > 1. */

unsigned char __fastcall__ VdpLayer(unsigned char layer, unsigned char on);
/* Show (on != 0) or hide layer 0-1, keeping LxCTRL's other bits.  Marks
 * VID_MODE disturbed.  Returns 1 if no card or layer > 1. */

int __fastcall__ VdpStatus(unsigned char reg);
/* Read status register 0-15 (VC_STAT*) on port A, or -1 if no card or
 * reg > 15.  Reading STAT0 clears its flags, and STAT1 its latches. */
 ...
 * the blanking interval rather than partway down the picture.  In this build
 * it is WaitVBlank(): it reads no flag anything else waits for, and with no
 * video card it waits 2 cs. */
 ...
 * and the support comes out of 6502-VDP.lib.
 *
 ...
 *               so it works on the serial console too.  textcolor() and
 *               bgcolor() set the pen, so they colour text written after the
 *               call; bordercolor() is bgcolor(), and revers() does nothing
 *               on screen — see the comment at the top of lib/conio.s.
 *
...
/* Two complete, independent port pairs, each with its own VRAM pointer,
 * command flip-flop and status select.  Use port A in foreground code and
 * port B in interrupt handlers; the Kernal never touches port B.
 * Register write:  value to VC_REG, then (register | VC_REG_WRITE) to VC_REG.
 * VRAM write:      address low to VC_REG, then (bits 13:8 | VC_ADDR_WRITE).
 * VRAM read:       address low to VC_REG, then bits 13:8.
 * Address bits 15:14 come from VC_REG_VBANK; the pointer moves by VC_REG_VINC
 * after each VC_DATA access.  Every Kernal entry assumes VBANK = 0, VINC = +1,
 * so put them back.  A status port reads the register its STATSEL names. */
#define VC_DATA             (*(volatile unsigned char *)0x9C00)  /* R/W  — Port A VRAM data */
#define VC_REG              (*(volatile unsigned char *)0x9C01)  /* Write— Port A command */
#define VC_STATUS           (*(volatile unsigned char *)0x9C01)  /* Read — Port A status (STATSEL_A) */
#define VC_DATA2            (*(volatile unsigned char *)0x9C02)  /* R/W  — Port B VRAM data (interrupt handlers) */
#define VC_REG2             (*(volatile unsigned char *)0x9C03)  /* Write— Port B command */
#define VC_STATUS2          (*(volatile unsigned char *)0x9C03)  /* Read — Port B status (STATSEL_B) */

/* Command byte bits: */
#define VC_REG_WRITE        0x80          /* %1rrrrrrr — write register r */
#define VC_ADDR_WRITE       0x40          /* %01aaaaaa — set the VRAM pointer for writing */

/* Register numbers (write-only).  $02-$06 alias L0NAME, L0ATTR, L0PAT,
 * SPRATTR and SPRPAT; these are the block addresses. */
#define VC_REG_MODE0        0x00          /* b1 M3 */
#define VC_REG_MODE1        0x01          /* DISP, IE, M1, M2, SPRSIZE, SPRMAG */
#define VC_REG_COLOR        0x07          /* b7:4 default foreground, b3:0 backdrop/border */
#define VC_REG_VBANK        0x08          /* VRAM address bits 15:14 */
#define VC_REG_VINC         0x09          /* Pointer stride, signed (reset +1) */
#define VC_REG_IRQEN        0x0A          /* Interrupt enables (VC_IRQ_*) */
#define VC_REG_IRQLINE      0x0B          /* Scanline compare */
#define VC_REG_PALBASE      0x0C          /* Palette base, x $400 (reset $3F = VC_PALETTE) */
#define VC_REG_VMODE        0x0D          /* Display mode (VC_VMODE_*) */
#define VC_REG_STATSEL_B    0x0E          /* Status register port B reads */
#define VC_REG_STATSEL_A    0x0F          /* Status register port A reads */
#define VC_REG_L0NAME       0x10          /* Layer 0 name table base, x $400 */
#define VC_REG_L0ATTR       0x11          /* Layer 0 attribute table base, x $400 */
#define VC_REG_L0PAT        0x12          /* Layer 0 pattern table base, x $800 */
#define VC_REG_L0SCRX       0x13          /* Layer 0 horizontal scroll, bits 7:0 */
#define VC_REG_L0SCRY       0x14          /* Layer 0 vertical scroll */
#define VC_REG_L0CTRL       0x15          /* Layer 0 control (VC_LCTRL_*; reset $3C) */
#define VC_REG_L0PAL        0x16          /* Layer 0 palette group high bits */
#define VC_REG_L1NAME       0x18          /* Layer 1 name table base, x $400 */
#define VC_REG_L1ATTR       0x19          /* Layer 1 attribute table base, x $400 */
#define VC_REG_L1PAT        0x1A          /* Layer 1 pattern table base, x $800 */
#define VC_REG_L1SCRX       0x1B          /* Layer 1 horizontal scroll, bits 7:0 */
#define VC_REG_L1SCRY       0x1C          /* Layer 1 vertical scroll */
#define VC_REG_L1CTRL       0x1D          /* Layer 1 control (reset $0C) */
#define VC_REG_L1PAL        0x1E          /* Layer 1 palette group high bits */
#define VC_REG_SPRATTR      0x20          /* Sprite attribute table base, x $80 */
#define VC_REG_SPRPAT       0x21          /* Sprite pattern table base, x $800 */
#define VC_REG_SPRCOUNT     0x22          /* Active sprite slots, 0-64 */
#define VC_REG_SPRCTRL      0x23          /* Sprite control (VC_SPRCTRL_*; reset $27) */
#define VC_REG_SPRLIMIT     0x24          /* Sprites drawn per scanline, 1-32 */
#define VC_REG_SPRPAL       0x25          /* Sprite palette group high bits */
#define VC_REG_FONT         0x30          /* Load a built-in font: b7 layer, b6:0 font ID (VC_FONT_*) */
 ...
 * one macro per address.  C only — 6502-VDP.inc has no counterpart, because
 ...
 * port, the ACIA status/reset port, the VDP command/status ports) appear once
...
/* --- Video Card (6502-PICOVDP) (IO 8: $9C00-$9C03) --- */
/* Two port pairs, and each command port reads as status.  A register write is
 * two bytes to a command port — the value, then the register number with
 * VC_REG_WRITE set:
 *
 *     VC.reg = TMS_BLACK;                        ~ border black
 *     VC.reg = VC_REG_WRITE | VC_REG_COLOR;
 *
 * Setting a VRAM address is the same shape: low byte, then bits 13:8 ORed with
 * VC_ADDR_WRITE to write or left bare to read, after which data moves by VINC.
 * An interrupt handler uses reg2/status2/data2, port B, so it cannot land
 * between the two writes of a foreground command on port A. */
struct __ac_vc {
    unsigned char       data;           /* +$00  R/W  Port A VRAM data */
    union {
        unsigned char   reg;            /* +$01  Write Port A command */
        unsigned char   status;         /* +$01  Read  Port A status */
    };
    unsigned char       data2;          /* +$02  R/W  Port B VRAM data */
    union {
        unsigned char   reg2;           /* +$03  Write Port B command */
        unsigned char   status2;        /* +$03  Read  Port B status */
    };
};
...
/* --- BIOS Version --- */
#define BIOS_VERSION_MAJOR  2
#define BIOS_VERSION_MINOR  0
...
#define VID_NAME_TABLE      0x0000        /* VRAM address of name table (40x24 = 960 bytes; L0NAME = $00) */
#define VID_ATTR_TABLE      0x0400        /* VRAM address of attribute table (960 bytes, fg<<4 | bg; L0ATTR = $01) */
#define VID_PATTERN_TABLE   0x0800        /* VRAM address of pattern table (2048 bytes; L0PAT = $01) */

/* --- 6502-PICOVDP: MODE0 / MODE1 bits --- */
/* M1, M2 and M3 select the display mode only while VMODE is VC_VMODE_LEGACY. */
#define VC_MODE0_M3         0x02          /* M3 legacy mode select */
#define VC_MODE1_DISP       0x40          /* Display enable */
#define VC_MODE1_IE         0x20          /* Vertical blank IRQ enable (alias of VC_IRQ_VBLANK) */
#define VC_MODE1_M1         0x10          /* M1 legacy mode select (Text) */
#define VC_MODE1_M2         0x08          /* M2 legacy mode select */
#define VC_MODE1_SPRSIZE    0x02          /* Sprite size: 0 = 8x8, 1 = 16x16 */
#define VC_MODE1_SPRMAG     0x01          /* Magnify sprites x2 */

/* --- 6502-PICOVDP: VMODE values --- */
#define VC_VMODE_LEGACY     0x0           /* M1/M2/M3 select the mode (reset) */
#define VC_VMODE_TEXT       0x1           /* 40x24 cells of 6x8, 240x192 */
#define VC_VMODE_COMPACT    0x2           /* 32x24 cells of 8x8, 256x192 */
#define VC_VMODE_GRAPHICS   0x3           /* 32x30 cells of 8x8, 256x240 */
#define VC_VMODE_FULL       0x4           /* 40x30 cells of 8x8, 320x240, no border */

/* --- 6502-PICOVDP: L0CTRL / L1CTRL bits --- */
/* Pick one depth and one attribute source, then OR in the flags. */
#define VC_LCTRL_1BPP       0x00          /* b1:0 bit depth — 8 bytes per tile */
#define VC_LCTRL_2BPP       0x01          /*   16 bytes per tile */
#define VC_LCTRL_4BPP       0x02          /*   32 bytes per tile */
#define VC_LCTRL_8BPP       0x03          /*   64 bytes per tile */
#define VC_LCTRL_ATTR_CELL  0x00          /* b3:2 attribute source — one byte per cell */
#define VC_LCTRL_ATTR_GROUP 0x04          /*   one byte per 8 patterns */
#define VC_LCTRL_ATTR_ROW   0x08          /*   one byte per pattern row */
#define VC_LCTRL_ATTR_NONE  0x0C          /*   no attribute fetch */
#define VC_LCTRL_ENABLE     0x10          /* b4 layer enable */
#define VC_LCTRL_OPAQUE     0x20          /* b5 index 0 opaque */
#define VC_LCTRL_SCRX8      0x40          /* b6 horizontal scroll bit 8 */

/* --- 6502-PICOVDP: attribute byte (layers at 2/4/8bpp, and sprites) --- */
#define VC_ATTR_SUBPAL      0x0F          /* b3:0 sub-palette */
#define VC_ATTR_FLIPH       0x10          /* b4 flip horizontally */
#define VC_ATTR_FLIPV       0x20          /* b5 flip vertically */
#define VC_ATTR_PRIORITY    0x40          /* b6 priority */
#define VC_ATTR_BIT8        0x80          /* b7 layer: pattern index bit 8; sprite: X bit 8 */

/* --- 6502-PICOVDP: SPRCTRL bits --- */
#define VC_SPRCTRL_ENABLE   0x01          /* b0 sprites enable */
#define VC_SPRCTRL_COLLIDE  0x02          /* b1 collision detection enable */
#define VC_SPRCTRL_TERM     0x04          /* b2 a Y of VC_SPRITE_END ends the list */
#define VC_SPRCTRL_DETAIL   0x08          /* b3 detailed collision map */
#define VC_SPRCTRL_1BPP     0x00          /* b5:4 sprite bit depth */
#define VC_SPRCTRL_2BPP     0x10
#define VC_SPRCTRL_4BPP     0x20
#define VC_SPRCTRL_8BPP     0x30

/* --- 6502-PICOVDP: interrupt sources (IRQEN bits, latched in STAT1) --- */
#define VC_IRQ_VBLANK       0x01          /* b0 end of the active picture */
#define VC_IRQ_SCANLINE     0x02          /* b1 display line matches IRQLINE */
#define VC_IRQ_OVERFLOW     0x04          /* b2 a sprite was dropped */
#define VC_IRQ_COLLISION    0x08          /* b3 two sprites collided */

/* --- 6502-PICOVDP: status registers (values for STATSEL_A / STATSEL_B) --- */
/* Reading STAT0 clears its flags, STAT7, the collision map and the vblank,
 * overflow and collision latches in STAT1.  Reading STAT1 clears only the
 * STAT1 latches. */
#define VC_STAT0            0             /* F, OVF, COL, first sprite dropped */
#define VC_STAT1            1             /* Latched interrupt sources */
#define VC_STAT2            2             /* Current display line, low 8 bits */
#define VC_STAT3            3             /* Blanking (VC_STAT3_*) */
#define VC_STAT4            4             /* Identification byte, VC_ID */
#define VC_STAT5            5             /* Firmware version, BCD */
#define VC_STAT6            6             /* Capabilities (VC_CAP_*) */
#define VC_STAT7            7             /* First sprite dropped on the latest overflowing line */
#define VC_STAT_COLLIDE     8             /* STAT8-STAT15: collision map */

#define VC_STAT0_F          0x80          /* A picture has ended */
#define VC_STAT0_OVF        0x40          /* A sprite has been dropped */
#define VC_STAT0_COL        0x20          /* Two sprites have collided */
#define VC_STAT0_SPRITE     0x1F          /* Low five bits of the first sprite dropped */

#define VC_STAT3_VBLANK     0x01          /* Vertical blanking */
#define VC_STAT3_HBLANK     0x02          /* Horizontal blanking (advisory) */

#define VC_ID               0xAC          /* STAT4 on a 6502-PICOVDP */

#define VC_CAP_LAYER2       0x01          /* STAT6 b0 two layers */
#define VC_CAP_8BPP         0x02          /* STAT6 b1 8bpp layer */
#define VC_CAP_FLIP         0x04          /* STAT6 b2 sprite flip */
#define VC_CAP_SCROLL       0x08          /* STAT6 b3 hardware scroll */
#define VC_CAP_SCANLINE     0x10          /* STAT6 b4 scanline IRQ */
#define VC_CAP_VRAM64K      0x20          /* STAT6 b5 64 KB VRAM */
#define VC_CAP_FONT         0x80          /* STAT6 b7 built-in font */

/* --- 6502-PICOVDP: FONT values --- */
/* OR a layer into a font ID: VC_FONT_LAYER1 | VC_FONT_CP437.  The load lands
 * at the next vertical blank; VdpLoadFont() waits for it. */
#define VC_FONT_LAYER0      0x00          /* b7 clear: layer 0's pattern table */
#define VC_FONT_LAYER1      0x80          /* b7 set: layer 1's pattern table */
#define VC_FONT_CP437       0x00          /* Font $00: CP437, 6x8, 2,048 bytes */

/* --- 6502-PICOVDP: VRAM --- */
#define VC_PALETTE          0xFC00        /* Palette at reset PALBASE: 256 entries of 0x0R, 0xGB */
#define VC_SPRITE_END       0xD0          /* Sprite Y that ends the list while VC_SPRCTRL_TERM is set */

/* --- Colour Constants (palette row 0) --- */
/* The PICOVDP's default palette puts the sixteen TMS9918 colours in row 0 at
 * their TMS9918 indices, so these name the same colours they always did.
 * Pass as (foreground << 4) | background, e.g. white on black = 0xF0. */
