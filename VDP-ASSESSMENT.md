...
- `6502.h` names `MONITOR_ENTRY`/`MONITOR_BRK_ENTRY`, and its `BRK_PTR` comment says
  "enters Monitor".
...
**Part 1 (BIOS 1.6):**
- Take the 1.6 legacy include from 6502-ASM.
- Add the 6 NVRAM slot prototypes to `6502.h`, with `__fastcall__` wrappers in `lib/`
  (a new `nvram.s`), so C programs on legacy machines can use save slots.
- `make check` stays green.

**Part 2 is blocked on 6502-ASM's convention** (and on BIOS 2.0 for Kernal entries).
...
4. **The VDP header drops the Monitor and the ROM font:** no `MONITOR_*`, no `$B800`
   character set. `BRK_PTR` and the BRK register comments describe the BRK report. Add a
   `VdpLoadFont` wrapper.
5. **Names.** Keep `TMS_*` for the legacy header. The VDP header needs palette-oriented
   names (row 0 keeps the same sixteen indices) and C names for the registers `$08`–`$7F`.
6. **Programs and README.** Build `HelloWorld`/`HelloWorldCart` both ways; `make run`
